...
from utils.shell import MODELS
from tools.config import Config
from tools.DataAugmentation import DataAugmentation as DA
from utils.mrcnnshell import ANCHORS
...
class MyFrame(wx.Frame):
    ...
    def unmold(self, bboxes, meta, mode = 'uxywh'):
        if mode == 'uxywh':
            ih, iw = meta[0][4:6]
            window = meta[0][7:11]
            scale = meta[0][11]
    ...
    def feature_extraction(self, input_img, input_box, meta):
        ...
        if self.config.M == 'mrcnn':
            ...
            detection = self.unmold(detection[0], meta)
        return feats[0], features[0], detection
    
    def load(self,):
        if hasattr(self, 'model'):
            del self.model
        models = MODELS(use_private_detector = True, config = self.config)
